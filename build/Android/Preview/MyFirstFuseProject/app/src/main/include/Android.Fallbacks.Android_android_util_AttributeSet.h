// This file was generated based on C:\ProgramData\Uno\Packages\Android\0.33.0\Android\Fallbacks\$.uno.
// WARNING: Changes might be lost if you edit this file directly.

#pragma once
#include <Android.android.util.AttributeSet.h>
#include <Android.Base.Wrappers.IJWrapper.h>
#include <Android.java.lang.Object.h>
#include <jni.h>
#include <Uno.IDisposable.h>
namespace g{namespace Android{namespace Fallbacks{struct Android_android_util_AttributeSet;}}}
namespace g{namespace Android{namespace java{namespace lang{struct String;}}}}
namespace g{namespace Android{namespace Runtime{struct ObjectArray;}}}

namespace g{
namespace Android{
namespace Fallbacks{

// public sealed extern class Android_android_util_AttributeSet :23077
// {
struct Android_android_util_AttributeSet_type : ::g::Android::java::lang::Object_type
{
    ::g::Android::android::util::AttributeSet interface2;
};

Android_android_util_AttributeSet_type* Android_android_util_AttributeSet_typeof();
void Android_android_util_AttributeSet__getAttributeBooleanValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, bool* arg2, bool* __retval);
void Android_android_util_AttributeSet__getAttributeBooleanValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, bool* arg1, bool* __retval);
void Android_android_util_AttributeSet__getAttributeBooleanValue_IMPL_20306_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, bool* arg4_, bool* __retval);
void Android_android_util_AttributeSet__getAttributeBooleanValue_IMPL_20312_fn(bool* arg0_, jobject* arg1_, int* arg2_, bool* arg3_, bool* __retval);
void Android_android_util_AttributeSet__getAttributeCount_fn(Android_android_util_AttributeSet* __this, int* __retval);
void Android_android_util_AttributeSet__getAttributeCount_IMPL_20299_fn(bool* arg0_, jobject* arg1_, int* __retval);
void Android_android_util_AttributeSet__getAttributeFloatValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, float* arg2, float* __retval);
void Android_android_util_AttributeSet__getAttributeFloatValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, float* arg1, float* __retval);
void Android_android_util_AttributeSet__getAttributeFloatValue_IMPL_20310_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, float* arg4_, float* __retval);
void Android_android_util_AttributeSet__getAttributeFloatValue_IMPL_20316_fn(bool* arg0_, jobject* arg1_, int* arg2_, float* arg3_, float* __retval);
void Android_android_util_AttributeSet__getAttributeIntValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, int* arg2, int* __retval);
void Android_android_util_AttributeSet__getAttributeIntValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, int* arg1, int* __retval);
void Android_android_util_AttributeSet__getAttributeIntValue_IMPL_20308_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, int* arg4_, int* __retval);
void Android_android_util_AttributeSet__getAttributeIntValue_IMPL_20314_fn(bool* arg0_, jobject* arg1_, int* arg2_, int* arg3_, int* __retval);
void Android_android_util_AttributeSet__getAttributeListValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, ::g::Android::Runtime::ObjectArray* arg2, int* arg3, int* __retval);
void Android_android_util_AttributeSet__getAttributeListValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, ::g::Android::Runtime::ObjectArray* arg1, int* arg2, int* __retval);
void Android_android_util_AttributeSet__getAttributeListValue_IMPL_20305_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, uObject* arg4_, int* arg5_, int* __retval);
void Android_android_util_AttributeSet__getAttributeListValue_IMPL_20311_fn(bool* arg0_, jobject* arg1_, int* arg2_, uObject* arg3_, int* arg4_, int* __retval);
void Android_android_util_AttributeSet__getAttributeName_fn(Android_android_util_AttributeSet* __this, int* arg0, ::g::Android::java::lang::String** __retval);
void Android_android_util_AttributeSet__getAttributeName_IMPL_20300_fn(bool* arg0_, jobject* arg1_, int* arg2_, uObject** __retval);
void Android_android_util_AttributeSet__getAttributeNameResource_fn(Android_android_util_AttributeSet* __this, int* arg0, int* __retval);
void Android_android_util_AttributeSet__getAttributeNameResource_IMPL_20304_fn(bool* arg0_, jobject* arg1_, int* arg2_, int* __retval);
void Android_android_util_AttributeSet__getAttributeResourceValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, int* arg2, int* __retval);
void Android_android_util_AttributeSet__getAttributeResourceValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, int* arg1, int* __retval);
void Android_android_util_AttributeSet__getAttributeResourceValue_IMPL_20307_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, int* arg4_, int* __retval);
void Android_android_util_AttributeSet__getAttributeResourceValue_IMPL_20313_fn(bool* arg0_, jobject* arg1_, int* arg2_, int* arg3_, int* __retval);
void Android_android_util_AttributeSet__getAttributeUnsignedIntValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, int* arg2, int* __retval);
void Android_android_util_AttributeSet__getAttributeUnsignedIntValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, int* arg1, int* __retval);
void Android_android_util_AttributeSet__getAttributeUnsignedIntValue_IMPL_20309_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, int* arg4_, int* __retval);
void Android_android_util_AttributeSet__getAttributeUnsignedIntValue_IMPL_20315_fn(bool* arg0_, jobject* arg1_, int* arg2_, int* arg3_, int* __retval);
void Android_android_util_AttributeSet__getAttributeValue_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, ::g::Android::java::lang::String** __retval);
void Android_android_util_AttributeSet__getAttributeValue1_fn(Android_android_util_AttributeSet* __this, int* arg0, ::g::Android::java::lang::String** __retval);
void Android_android_util_AttributeSet__getAttributeValue_IMPL_20301_fn(bool* arg0_, jobject* arg1_, int* arg2_, uObject** __retval);
void Android_android_util_AttributeSet__getAttributeValue_IMPL_20302_fn(bool* arg0_, jobject* arg1_, uObject* arg2_, uObject* arg3_, uObject** __retval);
void Android_android_util_AttributeSet__getClassAttribute_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String** __retval);
void Android_android_util_AttributeSet__getClassAttribute_IMPL_20318_fn(bool* arg0_, jobject* arg1_, uObject** __retval);
void Android_android_util_AttributeSet__getIdAttribute_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String** __retval);
void Android_android_util_AttributeSet__getIdAttribute_IMPL_20317_fn(bool* arg0_, jobject* arg1_, uObject** __retval);
void Android_android_util_AttributeSet__getIdAttributeResourceValue_fn(Android_android_util_AttributeSet* __this, int* arg0, int* __retval);
void Android_android_util_AttributeSet__getIdAttributeResourceValue_IMPL_20319_fn(bool* arg0_, jobject* arg1_, int* arg2_, int* __retval);
void Android_android_util_AttributeSet__getPositionDescription_fn(Android_android_util_AttributeSet* __this, ::g::Android::java::lang::String** __retval);
void Android_android_util_AttributeSet__getPositionDescription_IMPL_20303_fn(bool* arg0_, jobject* arg1_, uObject** __retval);
void Android_android_util_AttributeSet__getStyleAttribute_fn(Android_android_util_AttributeSet* __this, int* __retval);
void Android_android_util_AttributeSet__getStyleAttribute_IMPL_20320_fn(bool* arg0_, jobject* arg1_, int* __retval);

struct Android_android_util_AttributeSet : ::g::Android::java::lang::Object
{
    static jmethodID getAttributeBooleanValue_20306_ID_;
    static jmethodID& getAttributeBooleanValue_20306_ID() { return getAttributeBooleanValue_20306_ID_; }
    static jmethodID getAttributeBooleanValue_20312_ID_;
    static jmethodID& getAttributeBooleanValue_20312_ID() { return getAttributeBooleanValue_20312_ID_; }
    static jmethodID getAttributeCount_20299_ID_;
    static jmethodID& getAttributeCount_20299_ID() { return getAttributeCount_20299_ID_; }
    static jmethodID getAttributeFloatValue_20310_ID_;
    static jmethodID& getAttributeFloatValue_20310_ID() { return getAttributeFloatValue_20310_ID_; }
    static jmethodID getAttributeFloatValue_20316_ID_;
    static jmethodID& getAttributeFloatValue_20316_ID() { return getAttributeFloatValue_20316_ID_; }
    static jmethodID getAttributeIntValue_20308_ID_;
    static jmethodID& getAttributeIntValue_20308_ID() { return getAttributeIntValue_20308_ID_; }
    static jmethodID getAttributeIntValue_20314_ID_;
    static jmethodID& getAttributeIntValue_20314_ID() { return getAttributeIntValue_20314_ID_; }
    static jmethodID getAttributeListValue_20305_ID_;
    static jmethodID& getAttributeListValue_20305_ID() { return getAttributeListValue_20305_ID_; }
    static jmethodID getAttributeListValue_20311_ID_;
    static jmethodID& getAttributeListValue_20311_ID() { return getAttributeListValue_20311_ID_; }
    static jmethodID getAttributeName_20300_ID_;
    static jmethodID& getAttributeName_20300_ID() { return getAttributeName_20300_ID_; }
    static jmethodID getAttributeNameResource_20304_ID_;
    static jmethodID& getAttributeNameResource_20304_ID() { return getAttributeNameResource_20304_ID_; }
    static jmethodID getAttributeResourceValue_20307_ID_;
    static jmethodID& getAttributeResourceValue_20307_ID() { return getAttributeResourceValue_20307_ID_; }
    static jmethodID getAttributeResourceValue_20313_ID_;
    static jmethodID& getAttributeResourceValue_20313_ID() { return getAttributeResourceValue_20313_ID_; }
    static jmethodID getAttributeUnsignedIntValue_20309_ID_;
    static jmethodID& getAttributeUnsignedIntValue_20309_ID() { return getAttributeUnsignedIntValue_20309_ID_; }
    static jmethodID getAttributeUnsignedIntValue_20315_ID_;
    static jmethodID& getAttributeUnsignedIntValue_20315_ID() { return getAttributeUnsignedIntValue_20315_ID_; }
    static jmethodID getAttributeValue_20301_ID_;
    static jmethodID& getAttributeValue_20301_ID() { return getAttributeValue_20301_ID_; }
    static jmethodID getAttributeValue_20302_ID_;
    static jmethodID& getAttributeValue_20302_ID() { return getAttributeValue_20302_ID_; }
    static jmethodID getClassAttribute_20318_ID_;
    static jmethodID& getClassAttribute_20318_ID() { return getClassAttribute_20318_ID_; }
    static jmethodID getIdAttribute_20317_ID_;
    static jmethodID& getIdAttribute_20317_ID() { return getIdAttribute_20317_ID_; }
    static jmethodID getIdAttributeResourceValue_20319_ID_;
    static jmethodID& getIdAttributeResourceValue_20319_ID() { return getIdAttributeResourceValue_20319_ID_; }
    static jmethodID getPositionDescription_20303_ID_;
    static jmethodID& getPositionDescription_20303_ID() { return getPositionDescription_20303_ID_; }
    static jmethodID getStyleAttribute_20320_ID_;
    static jmethodID& getStyleAttribute_20320_ID() { return getStyleAttribute_20320_ID_; }

    bool getAttributeBooleanValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, bool arg2);
    bool getAttributeBooleanValue1(int arg0, bool arg1);
    int getAttributeCount();
    float getAttributeFloatValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, float arg2);
    float getAttributeFloatValue1(int arg0, float arg1);
    int getAttributeIntValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, int arg2);
    int getAttributeIntValue1(int arg0, int arg1);
    int getAttributeListValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, ::g::Android::Runtime::ObjectArray* arg2, int arg3);
    int getAttributeListValue1(int arg0, ::g::Android::Runtime::ObjectArray* arg1, int arg2);
    ::g::Android::java::lang::String* getAttributeName(int arg0);
    int getAttributeNameResource(int arg0);
    int getAttributeResourceValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, int arg2);
    int getAttributeResourceValue1(int arg0, int arg1);
    int getAttributeUnsignedIntValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1, int arg2);
    int getAttributeUnsignedIntValue1(int arg0, int arg1);
    ::g::Android::java::lang::String* getAttributeValue(::g::Android::java::lang::String* arg0, ::g::Android::java::lang::String* arg1);
    ::g::Android::java::lang::String* getAttributeValue1(int arg0);
    ::g::Android::java::lang::String* getClassAttribute();
    ::g::Android::java::lang::String* getIdAttribute();
    int getIdAttributeResourceValue(int arg0);
    ::g::Android::java::lang::String* getPositionDescription();
    int getStyleAttribute();
    static bool getAttributeBooleanValue_IMPL_20306(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_, bool arg4_);
    static bool getAttributeBooleanValue_IMPL_20312(bool arg0_, jobject arg1_, int arg2_, bool arg3_);
    static int getAttributeCount_IMPL_20299(bool arg0_, jobject arg1_);
    static float getAttributeFloatValue_IMPL_20310(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_, float arg4_);
    static float getAttributeFloatValue_IMPL_20316(bool arg0_, jobject arg1_, int arg2_, float arg3_);
    static int getAttributeIntValue_IMPL_20308(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_, int arg4_);
    static int getAttributeIntValue_IMPL_20314(bool arg0_, jobject arg1_, int arg2_, int arg3_);
    static int getAttributeListValue_IMPL_20305(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_, uObject* arg4_, int arg5_);
    static int getAttributeListValue_IMPL_20311(bool arg0_, jobject arg1_, int arg2_, uObject* arg3_, int arg4_);
    static uObject* getAttributeName_IMPL_20300(bool arg0_, jobject arg1_, int arg2_);
    static int getAttributeNameResource_IMPL_20304(bool arg0_, jobject arg1_, int arg2_);
    static int getAttributeResourceValue_IMPL_20307(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_, int arg4_);
    static int getAttributeResourceValue_IMPL_20313(bool arg0_, jobject arg1_, int arg2_, int arg3_);
    static int getAttributeUnsignedIntValue_IMPL_20309(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_, int arg4_);
    static int getAttributeUnsignedIntValue_IMPL_20315(bool arg0_, jobject arg1_, int arg2_, int arg3_);
    static uObject* getAttributeValue_IMPL_20301(bool arg0_, jobject arg1_, int arg2_);
    static uObject* getAttributeValue_IMPL_20302(bool arg0_, jobject arg1_, uObject* arg2_, uObject* arg3_);
    static uObject* getClassAttribute_IMPL_20318(bool arg0_, jobject arg1_);
    static uObject* getIdAttribute_IMPL_20317(bool arg0_, jobject arg1_);
    static int getIdAttributeResourceValue_IMPL_20319(bool arg0_, jobject arg1_, int arg2_);
    static uObject* getPositionDescription_IMPL_20303(bool arg0_, jobject arg1_);
    static int getStyleAttribute_IMPL_20320(bool arg0_, jobject arg1_);
};
// }

}}} // ::g::Android::Fallbacks
