// This file was generated based on C:\ProgramData\Uno\Packages\UnoCore\0.33.1\Backends\CPlusPlus\Uno\ObjectModel.cpp.
...
void uInitObjectModel()
{
    ...
    // Create Uno.Object, Uno.Type and Uno.Void type objects
...
static uType* uNewType(uint32_t type, const char* name, const uTypeOptions& options, bool isDefinition = true)
{
    ...
    if (isDefinition)
    {
        for (size_t i = 0; i < result->GenericCount; i++)
            result->Generics[i] = uGetGenericType(i);
...
static bool uIsMethodDefinition(uGenericType* type, size_t index)
{
    for (size_t i = index + 1; i < type->GenericCount; i++)
    {
        uGenericType* arg = (uGenericType*)type->Generics[i];
        if (arg->Type != uTypeTypeGeneric ||
            arg->GenericCount != 0 ||
            arg->GenericIndex != i)
            return false;
    }
...
uType* uNewMethodType(uType* parent, size_t index)
{
    ...
    // Avoid recursive generic method definition
    if (parent->GenericCount == 1 &&
        parent->Generics[0]->Type == uTypeTypeGeneric &&
        parent->Generics[0]->GenericCount &&
        parent->Generics[0]->Base->Definition == parent->Definition &&
        uIsMethodDefinition((uGenericType*)parent->Generics[0], index))
        return parent->Generics[0];

    uTypeOptions options;
    size_t count = index + 1;
    options.GenericCount = parent->GenericCount + count;
    options.DependencyCount = parent->DependencyCount;
    ...
    uGenericType* type =
        (uGenericType*)uNewType(
            uTypeTypeGeneric,
            (uBase::String(parent->FullName) + "``" + (int)count).CopyPtr(), // Leak
            options,
            false);
    type->Base = parent;
...
void uType::SetBase(uType* base)
{
    ...
    memcpy((uint8_t*)this + offset, (uint8_t*)base + offset, base->TypeSize - offset);
}
...
uType* uGetParameterized(uType* type, uType* root)
{
    ...
    {
        // Generic method
        uGenericType* generic = (uGenericType*)type;
        if (generic->GenericCount)
            break;

        U_ASSERT(generic->GenericIndex < root->GenericCount);
        return U_ASSERT_PTR(root->Generics[generic->GenericIndex]);
    }
    case uTypeTypeArray:
        return uGetParameterized(((uArrayType*)type)->ElementType, root)->Array();
    case uTypeTypeByRef:
        return uGetParameterized(((uByRefType*)type)->ValueType, root)->ByRef();
    default:
        break;
    }
...
static void uBuildParameterization(uType* type)
{
    ...
    {
        uType* parent = ((uGenericType*)type)->Base;
        if (!parent || def != type)
            parent = def;

        // Precalc generic method types
        parent->Build();
        U_ASSERT(type->DependencyCount == parent->DependencyCount &&
                 type->PrecalcCount == parent->PrecalcCount);
        for (size_t i = 0; i < type->DependencyCount; i++)
            type->DependencyTypes[i] = uGetParameterized(parent->DependencyTypes[i], type);
        for (size_t i = 0; i < type->PrecalcCount; i++)
            type->PrecalcTypes[i] = uGetParameterized(parent->PrecalcTypes[i], type);
    }
    else if (def == type)
    {
...
uType* uType::GetVirtual(uType* method)
{
    U_ASSERT(method && method->Type == uTypeTypeGeneric);
    uGenericType* gmethod = (uGenericType*)method;

    if (gmethod->Base == this)
        return method;

    size_t count = gmethod->GenericCount - gmethod->GenericIndex;
    U_ASSERT(count);
    size_t index = count - 1;
    U_ASSERT(index < MethodTypeCount);
...
uType* uType::MakeMethod(size_t count, uType* first, ...)
{
    U_ASSERT(count);
    size_t index = count - 1;
    U_ASSERT(index < MethodTypeCount);
...
uType* uType::U(size_t index)
{
    return Type == uTypeTypeGeneric
        ? T(((uGenericType*)this)->GenericIndex + index)
        : uGetGenericType(GenericCount + index);
}
...
bool uType::IsClosed()
{
    ...
    {
    case uTypeTypeByRef:
        return ((uByRefType*)this)->ValueType->IsClosed();
    case uTypeTypeArray:
        return ((uArrayType*)this)->ElementType->IsClosed();
...
{
    options.ValueSize = options.ObjectSize
        ? sizeof(uObject*)
        : 0; // "static class"
...
uEnumType* uEnumType::New(const char* name, uType* base, size_t literalCount)
{
    ...
    type->SetBase(_ObjectTypePtr); // We want to copy v-table from _ObjectTypePtr, so assign base after this.
    type->Base = base;
...
uDelegateType* uDelegateType::New(const char* name, size_t paramCount, size_t genericCount)
{
    ...
    type->SetBase(::g::Uno::Delegate_typeof());
